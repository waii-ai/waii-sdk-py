...
from tests.common_test_utils import connect_db, load_db_conn1
...
class TestDatabase(unittest.TestCase):
    ...
    def test_refresh_db_connection(self):
        ...
        for catalog in result.catalogs:
            for schema in catalog.schemas:
                for table in schema.tables:
                    if table.name.table_name == "test_refresh_db_connection":
                        table_exists = True
                        break
        ...
        if table_exists:
            try:
                WAII.Query.run(RunQueryRequest(query="DROP TABLE test_refresh_db_connection"))
            except Exception as e:
                print(f"Error dropping table: {e}")

        # now refresh the connection
